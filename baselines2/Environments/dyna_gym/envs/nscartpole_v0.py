...
class nscartpole_v0(gym.Env):
    metadata = {
        'render.modes': ['human', 'rgb_array'],
        'video.frames_per_second' : 50
    }

    def __init__(self,max_steps,speed):
        self.max_horizon = max_steps
        self.speed = speed
        ...
        self.force_mag = 10.0
        self.n_actions = 5 # number of discrete actions in [-force_mag,+force_mag]
        self.tau = speed * 0.02  # seconds between state updates

        # Angle at which to fail the episode
        self.theta_threshold_radians = 12 * 2 * math.pi / 360
        self.theta_dot_threshold_radian = 12 * 2 * math.pi / 360
        self.x_threshold = 0.1
        self.x_dot_threshold = 0.1
        ...
        self.high = np.array([
            self.x_threshold * 5,
            self.x_dot_threshold * 5,
            self.theta_threshold_radians * 5,
            self.theta_dot_threshold_radian * 5 ])


        self.action_space = Space(size=self.n_actions)
        self.action = spaces.Discrete(self.n_actions)
        self.observation_space = Space(low=-self.high, high=self.high, dtype=np.float32)
        self.time = None
    ...
    # def set_max_horizon(self,value):
    #     self.max_horizon
    #     return value
    ...
    def reset(self):
        self.state = \
            [self.np_random.uniform(low=-5*self.x_threshold, high=5*self.x_threshold),
             self.np_random.uniform(low=-5 * self.x_dot_threshold, high=5 * self.x_dot_threshold),
             self.np_random.uniform(low=-5 * self.theta_threshold_radians, high=5 * self.theta_threshold_radians),
             self.np_random.uniform(low=-5 * self.theta_dot_threshold_radian, high=5 * self.theta_dot_threshold_radian)
             ]
        # self.state = np.append(self.state,0) # do not append time
    ...
    def transition(self, state, action, is_model_dynamic):
        ...
        ## bound the state_p ##
        x = np.clip(x,-self.high[0],self.high[0])
        x_dot = np.clip(x_dot,-self.high[1],self.high[1])
        theta = np.clip(theta, -self.high[2], self.high[2])
        theta_dot = np.clip(theta_dot, -self.high[3], self.high[3])
        ...
        if self.non_stationary :
            ...
            self.time = 0
        success =  x < -self.x_threshold \
                or x > self.x_threshold \
                or theta < -self.theta_threshold_radians \
                or theta > self.theta_threshold_radians
        success = not bool(success)

        if not success:
            reward = -1
        else:
            reward = 100
            print("success!! get "+str(reward)+"rewards")
        ...
        '''
            ...
            reward = 0.0
        '''
        # print(state_p,self.time)
        return state_p, reward, success
