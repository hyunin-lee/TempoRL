...
class PredictEnv:
    ...
    def _termination_fn(self, env_name, obs, act, next_obs):
        ...
        if env_name == "Hopper-v2":
            ...
            z_coordinate = next_obs[:,0]
            not_done = np.logical_and(np.isfinite(z_coordinate) , np.logical_and(z_coordinate >= 0.2, z_coordinate <= 1.0))
            done = ~not_done
            done = done[:,None]
            return done 
        elif env_name == 'Humanoid-v2' : 
            z_coordinate = next_obs[:,0]
            done = np.logical_or(z_coordinate < 1.0 , z_coordinate > 2.0)
            done = done[:,None]
            return done
        elif env_name == 'Swimmer-v2' or 'HalfCheetah-v2' :
            done = np.zeros((next_obs[:,0].shape[0],),dtype=bool)
            done = done[:,None]
            return done
        else : 
